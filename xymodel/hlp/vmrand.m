...
% -- Short-cut, small fKappa
tbUniform = fKappa <= 5e-2;
tbDraw = true(size(tbUniform));
tbAccept = false(size(tbUniform));

% -- Rejection sampling loop
while (nnz(tbDraw & tbUniform) > 0)
   % - Draw partial variates and estimate wrapped Cauchy distribution envelope
   nNumToDraw = nnz(tbDraw & tbUniform);   
   tfZ(tbDraw & tbUniform) = rand(nNumToDraw, 1) * 2*pi - pi;
      
   vfRand2 = rand(nNumToDraw, 1);
   tbAccept(tbDraw & tbUniform) = exp(fKappa(tbDraw & tbUniform).*(cos(tfZ(tbDraw & tbUniform))-1)) - vfRand2 > 0;
   tfVMVariates(tbDraw & tbAccept) = tfZ(tbDraw & tbAccept);
